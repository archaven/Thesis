...
$color_car=$_POST['color_car'];
  if (empty ($farm_id_car)||empty ($type_car)||empty ($reg_car)||empty ($brand_car)||empty ($model_car)||empty ($color_car))
						{
...
{
echo "<script>alert('ขนาดไฟล์ภาพใหญ่เกินไปนะครับ');history.back();</script>";
	exit();
}
include "connect.php";
mysql_query("SET NAMES TIS620");
 $sql ="UPDATE car SET farm_id_car='$farm_id_car',type_car='$type_car',reg_car='$reg_car',brand_car='$brand_car',model_car='$model_car',color_car='$color_car' WHERE id_car='$id_car'";
...
echo "<script>alert('Complete!!');window.location='show_car.php?id=$_POST[id]';</script>";
mysql_close();
?>